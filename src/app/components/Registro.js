...
export default function Registro({ setRegistro }) {
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      // Abre el popup para que el usuario inicie sesión
      const result = await signInWithPopup(auth, provider);
      
      // Obtén información del usuario autenticado
      const user = result.user;
  
      // Verifica si el usuario ya existe en Firestore y si no, crea uno
      const userRef = doc(db, "users", user.uid);
      const userSnapshot = await userRef.get();
      
      if (!userSnapshot.exists()) {
        // Si no existe, crea un nuevo documento con la información del usuario
      ...
      <Formik
        ...
        onSubmit={async values => {
          ...
          try {
            ...
            console.log(error);
            Swal.fire({
              title: "Ups, hubo un error al registrar tu cuenta",
